...
func integration_MercuryV4(t *testing.T) {
	...
	for i := 0; i < nSrvs; i++ {
		...
		reqs := make(chan request, 100)
		srv := NewMercuryServer(t, ed25519.PrivateKey(k.Raw()), reqs, func() []byte {
			report, err := (&reportcodecv4.ReportCodec{}).BuildReport(ctx, v4.ReportFields{BenchmarkPrice: big.NewInt(234567), Bid: big.NewInt(1), Ask: big.NewInt(1), LinkFee: big.NewInt(1), NativeFee: big.NewInt(1), MarketStatus: 1})
			if err != nil {
				panic(err)
			}
			return report
